...
static inline char get_spi(void) { return SD_SELECT(); }
static inline char release_spi(void) { return SD_DESELECT(); }
  ...
  if (send_cmd(CMD0, 0) == 1) { /* Enter Idle state */
    ...
    if (send_cmd(CMD8, 0x1AA) == 1) { /* SDHC */
      ...
      if (ocr[2] == 0x01 && ocr[3] == 0xAA) {      /* The card can work at vdd range of 2.7-3.6V */
        ...
        while (sys_get_uptime_ms() < timeout && send_cmd(ACMD41, 1UL << 30))
          ; /* Wait for leaving idle state (ACMD41 with HCS bit) */

        if (sys_get_uptime_ms() < timeout && send_cmd(CMD58, 0) == 0) { /* Check CCS bit in the OCR */
...
DRESULT sd_read(BYTE *buff,   /* Pointer to the data buffer to store read data */
  ...
  if (g_disk_status & STA_NOINIT) return RES_NOTRDY;
  if (!get_spi()) return RES_ERROR;

  if (!(g_card_type & CT_BLOCK)) sector *= 512; /* Convert to byte address if needed */
...
DRESULT sd_write(const BYTE *buff, /* Pointer to the data to be written */
  ...
  if (!count) return RES_PARERR;
  if (g_disk_status & STA_NOINIT) return RES_NOTRDY;
  if (g_disk_status & STA_PROTECT) return RES_WRPRT;
  if (!get_spi()) return RES_ERROR;
  ...
  if (count == 1) { /* Single block write */
    ...
    if (g_card_type & CT_SDC) send_cmd(ACMD23, count);
  ...
  if (ctrl == CTRL_POWER) {
    switch (*ptr) {
      case 0:                          /* Sub control code == 0 (POWER_OFF) */
        if (SD_PRESENT()) power_off(); /* Power off */
        res = RES_OK;
        break;
