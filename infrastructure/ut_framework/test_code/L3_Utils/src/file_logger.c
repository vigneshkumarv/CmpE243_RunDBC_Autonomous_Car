...
static void logger_task(void *p) {
  ...
  while (1) {
     ...
     */
    log_msg = NULL;
    if (!xQueueReceive(g_write_buffer_queue, &log_msg, OS_MS(1000 * FILE_LOGGER_FLUSH_TIME_SEC)) || NULL == log_msg) {
 ...
 */
static bool logger_initialized(void) { return (NULL != gp_file_buffer); }
...
uint16_t logger_get_blocked_call_count(void) { return g_blocked_calls; }

uint16_t logger_get_highest_file_write_time_ms(void) { return g_highest_file_write_time; }

uint16_t logger_get_num_buffers_watermark(void) { return g_buffer_watermark; }
...
void logger_log(logger_msg_t type, const char *filename, const char *func_name, unsigned line_num, const char *msg,
  ...
  do {
    ...
    len = sprintf(buffer, "%d/%d,%02d:%02d:%02d,%u,%s,%s,%s%s,%u,", mon, day, hr, min, sec, up, log_type_str, filename,
                  func_name, func_parens, line_num);
  } while (0);
