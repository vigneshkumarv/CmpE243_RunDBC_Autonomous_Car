...
tlm_component *tlm_component_get_by_name(const char *name) {
  ...
  if (NULL != name) {
    comp = c_list_find_elm(mp_tlm_component_list, tlm_component_find_by_name, (void *)name, NULL, NULL);
  }
...
void tlm_component_for_each(tlm_comp_callback callback, void *arg1, void *arg2) {
   ...
   */
  if (NULL != callback) {
    c_list_for_each_elm(mp_tlm_component_list, tlm_component_for_each_callback, callback, arg1, arg2);
  }
}
