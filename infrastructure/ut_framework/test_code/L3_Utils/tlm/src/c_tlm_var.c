...
static bool tlm_variable_check_dup(void *elm_ptr, void *arg1, void *arg2, void *arg3) {
  tlm_reg_var_type *reg_var = elm_ptr;
  tlm_reg_var_type *new_var = arg1;

  return (reg_var->data_ptr != new_var->data_ptr && 0 != strcmp(reg_var->name, new_var->name));
}
...
bool tlm_variable_register(tlm_component *comp_ptr, const char *name, const void *data_ptr, const uint16_t data_size,
  ...
  if (!c_list_for_each_elm(comp_ptr->var_list, tlm_variable_check_dup, (void *)new_var, NULL, NULL)) {
    free(new_var);
    return false;
  }
...
const tlm_reg_var_type *tlm_variable_get_by_name(tlm_component *comp_ptr, const char *name) {
  ...
  if (NULL != comp_ptr && NULL != name && '\0' != *name) {
    reg_var = c_list_find_elm(comp_ptr->var_list, tlm_component_find_callback, (void *)name, NULL, NULL);
  }
  return reg_var;
}
