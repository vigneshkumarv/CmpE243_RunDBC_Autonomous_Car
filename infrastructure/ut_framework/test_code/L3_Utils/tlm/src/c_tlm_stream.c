...
static uint8_t hex_to_byte(const char *two_digit_hex) {
  ...
  if (isdigit(d2))
    ...
    value += tolower(d2) - 'a' + 10;

  return value;
}

static void tlm_stream_file_ptr(const char *str, void *fptr) { fwrite(str, sizeof(char), strlen(str), (FILE *)fptr); }
...
static bool tlm_stream_decode(FILE *file, tlm_component *p_comp) {
  ...
  if (NULL != reg_var && (reg_var->elm_size_bytes) * (reg_var->elm_arr_size) == exp_byte_cnt) {
...
void tlm_stream_one(tlm_component *comp, stream_callback_type stream, void *print_ascii, void *sca) {
   ...
   */
  c_list_for_each_elm((comp->var_list), tlm_stream_for_each_component_var, stream, /* arg1 */
                      sca,                                                         /* arg2 */
                      print_ascii                                                  /* arg3 */
  );
