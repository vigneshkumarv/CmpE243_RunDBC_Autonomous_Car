 ...
 */
static inline void ssp1_set_max_clock(unsigned int max_clock_mhz) { ssp_set_max_clock(LPC_SSP1, max_clock_mhz); }
 ...
 */
static inline char ssp1_exchange_byte(char out) { return ssp_exchange_byte(LPC_SSP1, out); }

/**
 * Writes a byte to the SPI FIFO
 * @warning YOU MUST ENSURE SPI IS NOT BUSY before calling this function
 */
static inline void ssp1_exchange_data(void* data, int len) { ssp_exchange_data(LPC_SSP1, data, len); }
