...
typedef struct {
  ...
  uint16_t droppedRxMsgs;  ///< Number of messages dropped if no space found during the CAN interrupt that queues the RX
                           ///< messages
...
bool CAN_tx(can_t can, can_msg_t *pCanMsg, uint32_t timeout_ms) {
  ...
  if (!ok) {
    ...
    do {
      can_msg_t msg;
      if (tx_all_avail == (CANx->SR & tx_all_avail) && xQueueReceive(pStruct->txQ, &msg, 0)) {
...
uint16_t CAN_get_rx_watermark(can_t can) { return CAN_VALID(can) ? CAN_STRUCT_PTR(can)->rxQWatermark : 0; }

uint16_t CAN_get_tx_watermark(can_t can) { return CAN_VALID(can) ? CAN_STRUCT_PTR(can)->txQWatermark : 0; }

uint16_t CAN_get_tx_count(can_t can) { return CAN_VALID(can) ? CAN_STRUCT_PTR(can)->txMsgCount : 0; }
...
can_fullcan_msg_t *CAN_fullcan_get_entry_ptr(can_std_id_t fc_id) {
  ...
  if (idx < num_entries) {
    ...
    can_fullcan_msg_t *base_msg_entry =
        (can_fullcan_msg_t *)(((uint8_t *)&(LPC_CANAF_RAM->mask[0])) + LPC_CANAF->ENDofTable);
...
#define CAN_ASSERT(x)                                                                                              \
  if (!(x)) {                                                                                                      \
    u0_dbg_printf(                                                                                                 \
        "Failed at %i, BUS: %s MOD: 0x%08x, GSR: 0x%08x\n"                                                         \
        "IER/ICR: 0x%08X/0x%08x BTR: 0x%08x"                                                                       \
        "\nLine %i: %s\n",                                                                                         \
        __LINE__, CAN_is_bus_off(can1) ? "OFF" : "ON", (int)LPC_CAN1->MOD, (int)LPC_CAN1->GSR, (int)LPC_CAN1->IER, \
