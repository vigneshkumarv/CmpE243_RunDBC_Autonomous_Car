...
#define portRESET_PRIVILEGE(xRunningPrivileged) \
  if (xRunningPrivileged != pdTRUE) __asm volatile(" mrs r0, control \n orr r0, #1 \n msr control, r0" ::: "r0")
 ...
 */
BaseType_t MPU_xTaskGenericCreate(TaskFunction_t pvTaskCode, const char *const pcName, uint16_t usStackDepth,
...
BaseType_t MPU_xQueueGenericReceive(QueueHandle_t pxQueue, void *const pvBuffer, TickType_t xTicksToWait,
...
BaseType_t MPU_xQueueAltGenericSend(QueueHandle_t pxQueue, const void *const pvItemToQueue, TickType_t xTicksToWait,
                                    BaseType_t xCopyPosition);
BaseType_t MPU_xQueueAltGenericReceive(QueueHandle_t pxQueue, void *const pvBuffer, TickType_t xTicksToWait,
                                       BaseType_t xJustPeeking);
...
void xPortPendSVHandler(void) {
  ...
  __asm volatile(
      ...
      "										\n" /* Restore the context. */
      "	ldr r1, [r3]						\n"
      "	ldr r0, [r1]						\n" /* The first item in the TCB is the task top of
                                                                       stack. */
      "	add r1, r1, #4						\n" /* Move onto the second item in the TCB... */
      "	ldr r2, =0xe000ed9c					\n" /* Region Base Address register. */
      "	ldmia r1!, {r4-r11}					\n" /* Read 4 sets of MPU registers. */
...
void vPortStoreTaskMPUSettings(xMPU_SETTINGS *xMPUSettings, const struct xMEMORY_REGION *const xRegions,
                               StackType_t *pxBottomOfStack, uint16_t usStackDepth) {
...
BaseType_t MPU_xTaskGenericCreate(TaskFunction_t pvTaskCode, const char *const pcName, uint16_t usStackDepth,
  ...
  xReturn = xTaskGenericCreate(pvTaskCode, pcName, usStackDepth, pvParameters, uxPriority, pxCreatedTask,
                               puxStackBuffer, xRegions);
  portRESET_PRIVILEGE(xRunningPrivileged);
  return xReturn;
}
/*-----------------------------------------------------------*/
