  ...
  if (xCommandID > tmrCOMMAND_START_DONT_TRACE) {                                                                 \
    if (xCommandID == tmrCOMMAND_CHANGE_PERIOD)                                                                   \
      ...
      trcKERNEL_HOOKS_TIMER_EVENT(                                                                                \
          EVENTGROUP_TIMER + xCommandID + ((xReturn == pdPASS) ? 0 : (TIMER_CREATE_FAILED - TIMER_CREATE)), tmr); \
    }                                                                                                             \
  }
...
#define traceEVENT_GROUP_WAIT_BITS_BLOCK(eg, bitsToWaitFor)                                                            \
  vTraceStoreKernelCallWithParam(EVENT_GROUP_WAIT_BITS_BLOCK, TRACE_CLASS_EVENTGROUP, TRACE_GET_EVENTGROUP_NUMBER(eg), \
                                 bitsToWaitFor);                                                                       \
  trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
...
#define traceEVENT_GROUP_WAIT_BITS_END(eg, bitsToWaitFor, wasTimeout)                                                  \
  if (wasTimeout) {                                                                                                    \
    ...
    vTraceStoreKernelCallWithParam(EVENT_GROUP_WAIT_BITS_END, TRACE_CLASS_EVENTGROUP, TRACE_GET_EVENTGROUP_NUMBER(eg), \
                                   bitsToWaitFor);                                                                     \
  }
...
#define traceEVENT_GROUP_CLEAR_BITS(eg, bitsToClear)                                                                \
  if (bitsToClear)                                                                                                  \
    vTraceStoreKernelCallWithParam(EVENT_GROUP_CLEAR_BITS, TRACE_CLASS_EVENTGROUP, TRACE_GET_EVENTGROUP_NUMBER(eg), \
                                   bitsToClear);
...
#define traceTASK_NOTIFY_TAKE()                                                                                        \
  if (pxCurrentTCB->ucNotifyState == taskNOTIFICATION_RECEIVED)                                                        \
    ...
    vTraceStoreKernelCallWithParam(TRACE_TASK_NOTIFY_TAKE_FAILED, TRACE_CLASS_TASK, uxTaskGetTaskNumber(pxCurrentTCB), \
                                   xTicksToWait);
...
#define traceTASK_NOTIFY_WAIT()                                                                                        \
  if (pxCurrentTCB->ucNotifyState == taskNOTIFICATION_RECEIVED)                                                        \
    ...
    vTraceStoreKernelCallWithParam(TRACE_TASK_NOTIFY_WAIT_FAILED, TRACE_CLASS_TASK, uxTaskGetTaskNumber(pxCurrentTCB), \
                                   xTicksToWait);
