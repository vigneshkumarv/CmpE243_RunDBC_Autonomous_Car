...
float reverse_low_speed = 14.1;
// 14.3 inconsistent,
// 14.1 works,
// 14.25 glitchy,
// 14.2 good but inconsistent on 4/8
...
void reverse_state_machine(void)  // every 100ms gets called
{
  if (state == 0) {
    ...
    Set_PWM_for_DC(stop_speed);  // stop
    state++;
  } else if (state == 1)  // 0
  {
    printf("second state (reverse)\n");
    Set_PWM_for_DC(reverse_low_speed);  // reverse
    state++;
  } else if (state == 2)  // 1
  {
...
void control_car_with_switches(void) {
  ...
  if (target_state == 1)  // low speed reverse
  {
    ...
    if ((reverse_cnt < 4)) {
      ...
      Set_PWM_for_DC(reverse_low_speed);
    }

  } else if (target_state == 2)  // stop
  {
    printf("Trying to stop...\n");
    if (Set_PWM_for_DC(stop_speed)) printf("Setting PWM3 to %f%%\n", stop_speed);
  ...
  } else if (target_state == 3)  // low speed forward
  {
    ...
    if (Set_PWM_for_DC(forward_low_speed)) printf("Setting PWM3 to %f%%\n", forward_low_speed);
  ...
  } else if (target_state == 4)  // medium speed forward
  {
    ...
    if (Set_PWM_for_DC(forward_medium_speed)) printf("Setting PWM3 to %f%%\n", forward_medium_speed);
