...
void receive_CAN_data_10Hz(void) {
    while (CAN_rx(can1, &geo_can_msg, 0)) {
        ...
        snprintf(RX_buffer, sizeof(RX_buffer), "~A%.2fD%0.2fL%fG%f#", to_android.deflection_angle, to_android.distance_from_destination, to_android.geo_src_latitude,
                to_android.geo_src_longitude);
